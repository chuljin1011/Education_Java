...
public class PenguinGameApp extends JFrame {
	...
	public class StoneThread extends Thread {
		...
		@Override
		public void run() {
			if (isPenguinAlive && isStonAlive) {
				if (isRun) {
					stoneY += 5;
				}

				if (stoneY + 20 >= penguinY) {
					if (stoneX + 10 >= penguinX && stoneX + 10 <= penguinX + PENGUIN_SIZE && stoneX + 20 >= penguinX
							&& stoneX + 20 <= penguinX + PENGUIN_SIZE) {
