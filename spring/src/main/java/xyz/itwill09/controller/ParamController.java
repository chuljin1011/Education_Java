...
public class ParamController {
	...
	*/
	/*
	//페이지 요청시 전달되는 값의 이름과 같은 이름으로 원시형(Wrapper 클래스) 또는 String 클래스
	//매개변수를 작성하면 Front Controller는 전달값을 반환받아 매개변수에 저장하여 사용할 수 있다.
	...
	*/
	...
	//@RequestMapping : 전달값을 제공받아 매개변수에 저장하기 위한 어노테이션
	...
	*/
	...
	//defaultValue 속성 : 매개변수에 저장될 기본값을 속성값을 설정
	...
	//전달값이 요소에 차례되로 저장된 배열을 제공받아 매개변수에 저장하여 사용
	@RequestMapping(value = "/paramValues", method = RequestMethod.POST)
	public String action2(@RequestParam("hobby") String[] hobbyList, Model model) {
	...
	*/
	...
	//전달값이 요소에 차례되로 저장된 List 객체를 제공받아 매개변수에 저장하여 사용
	@RequestMapping(value = "/paramValues", method = RequestMethod.POST)
	public String action2(@RequestParam("hobby") List<String> hobbyList, Model model) {
		model.addAttribute("hobbyList", hobbyList);
		return "paramValues_display";
	}
}
