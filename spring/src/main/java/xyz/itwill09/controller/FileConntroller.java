...
//3.MultipartHttpServletRequest 객체를 사용해 [multipart/form-data] 형식을 전달된 값 또는
//파일을 반환받아 처리
...
public class FileConntroller {
	...
	public String uploadOne(MultipartHttpServletRequest reqeust) throws IOException {
		String uploaderName=reqeust.getParameter("uploaderName");
		
		//MultipartHttpServletRequest.getFile(String name) : 전달파일의 정보가 저장된
		//MultipartFile 객체를 반환하는 메소드
		MultipartFile uploaderFile = reqeust.getFile("uploaderFile");
		
		//MultipartFile 객체에 저장된 전달파일에 대한 검증
		//MultipartFile.isEmpty() : MultipartFile 객체에 파일정보가 저장되어 있지 않은 경우 [false]를
		...
		String uploadDirectory = reqeust.getServletContext().getRealPath("/resources/images/upload");
		...
		reqeust.setAttribute("uploaderName", uploaderName);
		reqeust.setAttribute("uploadFileName", uploadFileName);
	...
	*/
	...
	//문제점 : 전달파일의 이름과 같은 이름의 파일이 서버 디렉토리에 있는 경우 기존 파일 대신
	//전당파일로 덮어씌워 저장(OverWrite)
	//해결법 : 전달파일을 이름으로 서버 디렉토리에 저장하지 않고 중복되지 않는 파일의 이름으로
	//저장되도록 변경하여 업로드 처리
	@RequestMapping(value = "/upload1", method = RequestMethod.POST)
