...
//=> [src/test/resource] 패키지 폴더에 [log4j.xml] 파일의 내용 변경
//3.[src/test/java] 패키지 폴더에 테스트 클래스(테스트 프로그램) 작성
// => junit 라이브러리와 spring-test 라이브러리의 scope 속성을 주석 처리한 후 클래스 작성
//4. 테스트 프로그램 실행

//@RunWith : 테스트 클래스로 테스트 프로그램을 실행하기 위한 클래스를 설정하는 어노테이션
// => 테스트 클래스를 객체로 생성하여 테스트 메소드를 호출해 테스트 명령
...
//@ContextConfiguration : 스프링 컨테이너에게 제공된 Spring Bean Configuration file을 제공하는 어노테이션
//locations 속성 : Spring Bean Configuration File 경로를 속성값으로 설정
// => Spring Bean Configuration File이 여러개인 경우 {} 기호를 사용해 배열 형식으로 속성값 제공
// => Spring Bean Configuration File의 경로는 file 접두사를 사용해 파일 시스템 경로로 제공
